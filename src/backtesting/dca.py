...
def dca(prices, indicator=None, income = 100, income_period = 21, invest_period=21, start_capital=1000, start_date = 0, allow_sell=False):
  portfolio_value = []
  shares = 0
  ...
  for days in range(start_date, len(prices)):
    ...
    should_act = allow_sell or (days % invest_period == 0)
    
    if should_act and not pd.isna(target):
      shares, not_invested_capital = allocate(shares, not_invested_capital, prices.iloc[days], target, allow_sell)
...
def allocate(shares, capital, price, target, allow_sell=False):
